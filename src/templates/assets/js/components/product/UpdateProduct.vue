...
export default {
  ...
  methods: {
    ...
    saveProduct() {
      ...
      axios.patch(`/product/api/product/${product_id}/`).then(response => {
  ...
  mounted() {
    ...
    axios.get(`/product/api/product/${product_id}`)
        .then(response => {
          this.info = response;
          console.log(response.data);
          ...
          this.product_variant = response.data.product_variant_one;
        });
  },
};
</script>
